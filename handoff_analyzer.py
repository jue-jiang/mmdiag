...
class HandoffAnalyzer(Analyzer):
    ...
    dictMCCMNC = {
        # ("208", "1"): "orange_sa",
        # ("214", "1"): "vodafone_spain",
        # ("214", "3"): "orange_espagne_sau",
        # ("262", "1"): "deutsche_telekom",
        ("310", "120"): "sprint",
        ("310", "260"): "tmobile",
        ("310", "410"): "att",
        ("311", "480"): "verizon",
    ...
    def __msg_callback(self,msg):
        ...
        if msg.type_id == "LTE_RRC_OTA_Packet":
            physicalCellID = log_item['Physical Cell ID']
            freq = log_item['Freq']
            timestamp = log_item['timestamp']
            print physicalCellID, freq, timestamp
        elif msg.type_id == "LTE_RRC_Serv_Cell_Info":
            physicalCellID = log_item['Cell ID']
            freq = log_item['Downlink frequency']
            timestamp = log_item['timestamp']
            longCellID = log_item['Cell Identity']
            MCC = str(log_item['MCC'])
            MNC = str(log_item['MNC'])
            if (MCC, MNC) not in self.dictMCCMNC:
                carrier = "Unknown"
            else:
                carrier = self.dictMCCMNC[(MCC, MNC)]
            print physicalCellID, freq, timestamp, longCellID, carrier, MCC, MNC
